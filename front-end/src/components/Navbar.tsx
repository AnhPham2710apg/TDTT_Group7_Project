    ...
    <nav className="border-b border-border bg-card/95 backdrop-blur-md sticky top-0 z-50 transition-all">
      <div className="container mx-auto px-4 py-3">
        ...
        <div className="flex items-center justify-between relative h-10">
          ...
          {/* --- 1. MOBILE HAMBURGER BUTTON (Chuyển sang TRÁI trên Mobile) --- */}
          {isLoggedIn && (
          ...
          {/* --- 2. LOGO (CĂN GIỮA TRÊN MOBILE / CĂN TRÁI TRÊN PC) --- */}
          <Link 
            to="/" 
            className="flex items-center gap-2 group z-50 absolute left-1/2 -translate-x-1/2 md:static md:translate-x-0 md:left-auto"
          >
            ...
            <span className="text-lg md:text-xl font-bold bg-hero-gradient bg-clip-text text-transparent whitespace-nowrap hidden min-[350px]:block">
              Food Tour Assistant
            </span>
          </Link>

          {/* --- 3. RIGHT ACTIONS (PC Menu + Cart + User) --- */}
          <div className="flex items-center gap-2 ml-auto"> {/* Thêm ml-auto để đẩy về bên phải */}
            {isLoggedIn ? (
              <>
                {/* PC MENU ITEMS */}
                ...
                <Button
                    ...
                    size="sm"
                    className={cn(
                      "relative h-9 w-9 md:h-10 md:w-10 rounded-full transition-all duration-300", // Giảm size nhẹ trên mobile
                      isCartActive 
              ...
              !hideAuthButtons && (
                <div className="flex items-center gap-2">
                  ...
                  <Button onClick={() => navigate("/register")} className="bg-hero-gradient hover:opacity-90">Sign Up</Button>
                </div>
              )
            )}
          </div>
