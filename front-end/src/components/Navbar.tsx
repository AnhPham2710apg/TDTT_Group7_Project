...
import { Avatar, AvatarFallback, AvatarImage } from "./ui/avatar"; // [NEW] AvatarImage
import {
...
const Navbar = ({ hideAuthButtons = false }: NavbarProps) => {
  ...
  const navigate = useNavigate();
  const location = useLocation();
  const { isLoggedIn, username, logout, avatarUrl } = useAuth(); // [NEW] avatarUrl
