...
def get_hcmc_grid():
    ...
    # Lưu ý: Bạn đã comment các quận/huyện, tôi giữ nguyên theo file của bạn
    # Để quét toàn bộ, hãy bỏ comment tất cả các dòng
    return grid
...
def get_search_terms():
    ...
    return [
        # === LOẠI HÌNH CHUNG ===
        'nhà hàng', 'quán ăn', 'quán cơm', 'tiệm ăn', 'quán nhậu', 'quán nước',
        'cà phê', 'tiệm trà', 'trà sữa', 'tiệm bánh', 'ăn vặt', 'ăn đêm', 'cơm văn phòng',
        'buffet', 'lẩu', 'nướng', 'hải sản', 'ốc', 'đặc sản', 'ẩm thực', 'quán chay', 'trà',
        
        # === MÓN ĂN VIỆT NAM (THEO MÓN) ===
        # Phở & Bún & Mì
        'phở', 'phở bò', 'phở gà', 'bún bò', 'bún bò Huế', 'bún đậu', 'bún đậu mắm tôm',
        'bún chả', 'bún riêu', 'bún mắm', 'bún thịt nướng', 'bún cá', 'hủ tiếu', 
        'hủ tiếu nam vang', 'hủ tiếu gõ', 'mì quảng', 'cao lầu', 'bánh canh', 'bánh canh cua',
        'bánh canh ghẹ',
        
        # Cơm
        'cơm tấm', 'cơm sườn', 'cơm gà', 'cơm gà xối mỡ', 'cơm chiên', 'cơm niêu',
        
        # Bánh
        'bánh mì', 'bánh mì chảo', 'bánh mì que', 'bánh xèo', 'bánh khọt', 'bánh cuốn',
        'bánh bèo', 'bánh bột lọc', 'bánh nậm', 'bánh giò', 'bánh bao',
        
        # Gỏi & Khai vị
        'gỏi cuốn', 'chả giò', 'gỏi',
        
        # Khác
        'cháo', 'cháo lòng', 'cháo gà', 'xôi', 'xôi mặn', 'xôi gà', 'bò kho', 'bò né',
        'phá lấu', 'cút lộn xào me',
        
        # === ẨM THỰC QUỐC TẾ ===
        'món Hàn', 'BBQ Hàn Quốc', 'cơm trộn', 'tokbokki', 'gà rán', 'gà rán Hàn Quốc',
        'món Nhật', 'sushi', 'sashimi', 'ramen', 'udon', 'cơm Nhật',
        'món Trung', 'dimsum', 'hủ tiếu mì', 'mì vịt tiềm', 'cơm chiên Dương Châu',
        'món Thái', 'lẩu Thái', 'gỏi Thái', 'pad Thái', 'cơm Thái',
        'món Ý', 'pizza', 'mì Ý', 'pasta', 'spaghetti',
        'món Âu', 'steak', 'bít tết',
        'món Ấn', 'cà ri', 'món Mexico', 'món Pháp',

        # === ĐỒ UỐNG & TRÁNG MIỆNG ===
        'trà sữa', 'trà đào', 'trà trái cây', 'nước ép', 'sinh tố',
        'kem', 'chè', 'tàu hũ', 'bánh flan', 'rau câu', 'yaourt',
        'bia', 'bia hơi', 'bia thủ công', 'bar', 'pub',
        'tiệm bánh ngọt', 'bánh kem', 'bánh donut', 'croissant',
        
        # === LOẠI HÌNH CỤ THỂ ===
        'fine dining', 'nhà hàng sang trọng', 'quán bar', 'beer club', 
        'cà phê sách', 'cà phê thú cưng', 'cà phê sân vườn'

        # === QUÁN CÀ PHÊ THƯƠNG HIỆU ===
        'Highlands Coffee', 'The Coffee House', 'Starbucks', 'Phúc Long',
        'Cộng Cà Phê', 'Katinat', 'Cafe Amazon', 'Trung Nguyên Legend',
        'Urban Station', 'Papaxốt Coffee', 'Coffee Bean & Tea Leaf',
        'Passio Coffee', 'Guta Coffee', 'Nenem Coffee', 'Oromia Coffee',
        'TocoToco', 'Koi Thé', 'Gong Cha', 'Tee Talk', 'Heekcaa', 'Sharetea',
        'Ding Tea', 'Bobapop', 'Royal Tea',

        # === QUÁN ĂN NHANH – FAST FOOD ===
        'KFC', 'McDonalds', 'Lotteria', 'Jollibee', 'Burger King',
        'Pizza Hut', 'Domino\'s Pizza', 'Texas Chicken', 'Popeyes',
        'Bánh Mì PewPew', 'Bánh Mì Xin Chào',

        # === THƯƠNG HIỆU ĐỒ ĂN NHẸ / CÀ PHÊ TAKE AWAY ===
        'BreadTalk', 'Tous Les Jours', 'Dunkin Donuts', 'Starbucks Reserve',
        'Marukame Udon', 'Sushi Kei', 'Hot & Cold', 'Katinat Saigon Kafé',

        # === THƯƠNG HIỆU TRÀ SỮA (ĐÃ CÓ NHƯNG BỔ SUNG THÊM) ===
        'Tiger Sugar', 'Trà sữa Mixue', 'Feeling Tea', 'ChaGo Tea & Café',
        'Holliday Tea', 'Maku Tea', 'Heytea'
    ]
...
def call_api(session, url, params):
    ...
    try:
        ...
        if response.status_code == 429:
            print("\n!!! LỖI NGHIÊM TRỌNG: ĐÃ ĐẠT GIỚI HẠN YÊU CẦU (429)!!!")
            print("Nguyên nhân: Rất có thể bạn đã hết 1000 yêu cầu/ngày.")
            print("Nhờ có 'INSERT OR IGNORE', kịch bản sẽ tự động tiếp tục phần còn lại vào lần chạy sau.")
            raise RateLimitError("Đã đạt giới hạn yêu cầu 429") # Ném lỗi tùy chỉnh
...
def start_scan():
    ...
    for area in grid:
        ...
        for term in search_terms:
            print(f"Đang quét: Từ khóa='{term}', Khu vực='{area['name']}'...")
            ...
            try:
                ...
                print("Lỗi 429 xảy ra trong Giai đoạn 1. Dừng quét Giai đoạn 1.")
                rate_limit_hit = True # Đặt cờ
                break # Thoát vòng lặp 'term'
            
            if data and data.get('status') == 'OK' and data.get('predictions'):
                count = 0
                for pred in data['predictions']:
                    if pred.get('compound', {}).get('province') == 'Hồ Chí Minh':
                        unique_place_ids.add(pred['place_id'])
                        count += 1
                print(f"    -> Tìm thấy {count} kết quả liên quan ở TP.HCM.")

    print(f"\n--- Giai đoạn 1 Hoàn tất: Tìm thấy {len(unique_place_ids)} địa điểm duy nhất. ---")
    ...
    try:
        ...
        for i, place_id in enumerate(list(unique_place_ids)):
            print(f"Đang xử lý {i+1}/{len(unique_place_ids)}: {place_id[:20]}...")
    ...
    print(f"Quá trình quét hoàn tất. Đã thêm {total_new_places} địa điểm mới vào database.")
